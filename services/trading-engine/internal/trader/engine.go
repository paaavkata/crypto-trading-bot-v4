...
func (e *Engine) processPair(ctx context.Context, pair models.SelectedPair) error { // pair is models.SelectedPair
	...
	// To use the value from tempSymbolPriceMap if available from global PnL update
	currentPriceFloat, ok := tempSymbolPriceMap[pair.Symbol]
	if !ok {
		price, err := e.repo.GetLatestPrice(ctx, pair.Symbol)
		if err != nil {
			return fmt.Errorf("failed to get current price for %s: %w", pair.Symbol, err)
		}
		currentPriceFloat = price
		tempSymbolPriceMap[pair.Symbol] = currentPriceFloat // Cache for this cycle if not already there
	}
	...
	for _, pos := range positions { // pos is a copy
		...
		stoppedOut, errSL := e.riskManager.shouldStopLoss(pos, currentPriceFloat) // pos is a copy, its PnL is current
		if errSL != nil {
			e.logger.WithError(errSL).WithFields(logrus.Fields{
				"position_id": pos.ID,
				"symbol":      pair.Symbol,
			}).Error("Error checking stop-loss condition")
			survivingPositions = append(survivingPositions, pos) // Keep if check fails
			continue
		}
		...
		tookProfit, errTP := e.riskManager.shouldTakeProfit(pos, currentPriceFloat)
		if errTP != nil {
			e.logger.WithError(errTP).WithFields(logrus.Fields{
				"position_id": pos.ID,
				"symbol":      pair.Symbol,
			}).Error("Error checking take-profit condition")
			survivingPositions = append(survivingPositions, pos) // Keep if check fails
			continue
		}
